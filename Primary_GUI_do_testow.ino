...
// ******************** sprawdzanie stanu SENSOR1 *********************
int supla_DigitalRead(int channelNumber, uint8_t pin) {
  int result = digitalRead(pin);
     if (pin == SENSOR1_PIN) {
        if(result == 1) {            // jeżeli drzwi otwarte to
        pompka_ON_OFF();
...
void pompka_ON_OFF() {
// Serial.print("ds18b20_channel[0]"); Serial.print(" ,Adres 0: "); Serial.print(ds18b20_channel[0].address); Serial.print(" ,Temp cyrkulacja: "); Serial.println(ds18b20_channel[0].last_val);  // temp powrotu cyrkulacji
// Serial.print("ds18b20_channel[1]"); Serial.print(" ,Adres 1: "); Serial.print(ds18b20_channel[1].address); Serial.print(" ,Temp bojlera: "); Serial.println(ds18b20_channel[1].last_val);      // temp bojlera
// Serial.print("Stan PIN pompki: "); Serial.println(digitalRead(RELAY1_PIN));

    if (ds18b20_channel[0].last_val >= (ds18b20_channel[1].last_val-2)) { //  temp (powrot cyrkulacji ) większa lub równa  ( BOJLER  - 2 STOPNIE)
        Serial.print("ds18b20_channel[0]"); Serial.print(" ,Adres 0: "); Serial.print(ds18b20_channel[0].address); Serial.print(" ,Temp cyrkulacja: "); Serial.println(ds18b20_channel[0].last_val);
        Serial.print("ds18b20_channel[1]"); Serial.print(" ,Adres 1: "); Serial.print(ds18b20_channel[1].address); Serial.print(" ,Temp bojlera: "); Serial.println(ds18b20_channel[1].last_val);      // temp bojlera
        ...
        SuplaDevice.relayOn( 0,0 ); // włącz relay1 z pomką i działa do momentu spełnienia warunku
        Serial.print("Włączam pompkę: "); Serial.println(digitalRead(RELAY1_PIN));
        }
     }
...
Serial.println (WiFi.status());
   if ((WiFi.status() == WL_CONNECTED) && b1 == 1) {
  Pushover po = Pushover("API","ID"); 
  po.setDevice("Slawek");
  po.setMessage("test");
